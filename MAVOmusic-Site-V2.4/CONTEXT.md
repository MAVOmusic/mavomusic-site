...
- **Schedule engine (V1.12, index):** STREAM_PLAN map "YYYY-MM-DD":[startH,endH) UK wall
  time (26 = 2AM next day) drives BOTH the watchnow banner (green LIVE text during a
  window, amber NEXT STREAM IN hh:mm:ss otherwise + "while you wait - check out the
  videos" line) AND the calendar rows in the new SCHEDULE/FEEDBACK tabber (schedule pane
  = default; feedback form moved inside #fbPane). Fallback when no plan day matches:
  daily 14-17 UK. Intl Europe/London; marathon spillover handled via yesterday's window.
  EDIT HERE comment for future weeks sits above STREAM_PLAN.
...
index shows "Total videos in the spotlight: N" from it)** + `READ-ME.md` (owner's manual).
