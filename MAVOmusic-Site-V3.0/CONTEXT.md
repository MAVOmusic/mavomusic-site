...
*Last updated: 2026-08-24 · Current site version: V3.0*

## Current state (V3.0) - read me first
- **REQUEST TOKENS ON SONG SEARCH (V3.0):** every library row shows a green
  "✓ FREE" chip (up to 6:00) or a gold "⭐ X TOKENS" chip with the exact
  StreamerSonglist minAmount (window.MAVO_PAID in songlist.js, mirrored from
  the Main Library CSVs - 4,126 paid, 44,242 free). Explainer added under the
  Song Search intro (prices 500 for 6:01-10:00, 1000 for 10:01-15:00 etc;
  earning rules both platforms). VAULT: index.json built final (56,084 files,
  56,084 chips, 47,828 songs, 0 mystery) + CORS-fixed worker live on
  files.mavomusic.co.uk; full sync to R2 bucket mavomusic-cdlc IN PROGRESS
  (~200GB, resumable). Owner mode = 5 taps on ver chip + ADMIN_KEY, chips
  pre-exist and say "not found" until each file lands - by design.

## Previous state (V2.9)
- **HOME RADIO BAR:** fixed slim bottom row on index.html - 🔊 ⏮ ⏯ ⏭ +
  title + "0:45 / 3:11" + full-bleed tap-to-seek line (pointerdown fraction).
  Long-form only, shuffle-bag, hidden 16px player sliver, unmuted-attempt →
  muted fallback + pulsing 🔊. SILENCE model (V2.9): a `quiet` latch +
  transient guards. Silenced by: routeToStage (the 12 cards in offline
  mid-stage mode - THE bug fix), loadCategory, loadLive, theatre overlay
  (also hides the bar). Stays paused after - only the visitor's ▶ (wake)
  resumes; closing the overlay only unhides. onState re-pauses any PLAYING
  that races past the latch. ⏮ uses a hist stack (restart when empty);
  mount() = cueVideoById while quiet, loadVideoById while live.
  Time format everywhere = "0:15 / 2:52" (radio.html + bass-radio.html had
  the fused "0:152:52" bug - fixed with a .tsep slash). Pick & Play cards
  (both columns) carry .hot pulsing green + "⭐ your press = one real view".
- **CDLC ADMIN MODE (V2.8 foundation):** five taps on .ver chip → prompt for
  owner key → localStorage mavo_cdlc_key → fetch files.mavomusic.co.uk/
  index.json → search rows gain .dl chips per version (href /f/<path>?k=key,
  download attr). window.MAVO_cdlcChips(row, norm, display) is called from
  wireSearch; visitors without a key see zero difference. Backend kit (NOT
  in the deploy zip): _To Attach/ADMIN/ = worker.js (R2 streamer, ADMIN_KEY
  secret, CDLC binding), CDLC-SETUP.md (full one-time guide + price honesty:
  R2 free to 10 GB, ~£2.40/mo @ 200 GB), CDLC-Index-Builder.ps1 (folder
  scan → index.json, "Artist - Title" + version tails). Owner has NOT yet
  created the R2 side - site chips stay dormant until he does.
