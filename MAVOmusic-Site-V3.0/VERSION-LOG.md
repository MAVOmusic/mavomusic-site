...
## V3.0 - 2026-08-24
- **Song Search now prices every request (V3.0):** every library row carries
  a request-cost chip - green "✓ FREE" for anything up to 6 minutes, gold
  "⭐ X TOKENS" with the EXACT price for the 4,126 longer epics. Prices are
  not computed on the site: songlist.js now also exports window.MAVO_PAID,
  a "norm key|tokens" map mirrored 1:1 from StreamerSonglist's own minAmount
  column in the two Main Library CSVs, so the row can never disagree with
  what the request bot will actually deduct. Absent from the map = free
  (44,242 songs). New explainer paragraph under the Song Search intro:
  free up to 6:00, 500 tokens for 6:01-10:00, 1000 for 10:01-15:00 and up
  for the monsters; earning = 500 per sub/gifted sub, 1 per Kick or Bit,
  channel point rewards, 100 for first chat message per day, both platforms;
  StreamerSonglist checks the balance and deducts on request. Version chip
  bumped to SITE V3.0. jsdom 4/4 (paid chip exact amount, free chip, !sr
  command intact, explainer copy present).

## V2.9 - 2026-08-23
- **Radio-stop bug fixed (owner's report: "both kept playing"):** V2.8 only
  hooked the full-screen overlay - but when the stream is offline the site
  serves the 12 menu cards into the MIDDLE stage (routeToStage, loadStage),
  which never touched the radio, so both sounds ran together. Silence hooks
  now also live in routeToStage (the 12 cards), loadCategory (shelf tabs) and
  loadLive (a live broadcast owns the air). Plus a belt-and-suspenders guard:
  any PLAYING state that sneaks past a silence latch (slow-boot race,
  autoplay-on-ready) is re-paused instantly. After any of this the radio
  stays paused - the visitor's own play button brings it back, so nobody
  gets surprise double audio behind the Marathon.
- **The bar shrank into a real deck:** one slim row - 🔊 ⏮ ⏯ ⏭, then the
  song title, then mm:ss / mm:ss - about half the old height, more room for
  the page. Back button pulls from song history, pause/resume toggles, and
  the bottom line is now a tap-to-seek scrubber: touch anywhere, hear that
  second (0:45, 1:26, 3:11). Transport sits before the band name, the clock
  after it - exactly the requested order. Full viewport width rules and the
  hidden video sliver unchanged.
- Chip SITE V2.9. Battery clean; jsdom E2E 20/20 including a scripted replay
  of the exact 12-menu-click bug, the slow-boot race, visitor resume, history
  back/next, mid-bar seeking and the CDLC owner chip.

## V2.8 - 2026-08-23
- **Ambient home radio (MySpace mode):** a thin bar pinned to the bottom of the
  home screen plays full-length long-form songs (longs only - shorts keep
  their own screens) while visitors browse: random shuffle-bag order (every
  song once before any repeat), continuous track-after-track, title marquee,
  and a full-bleed progress line that always spans the whole viewport - zoom
  in, zoom out, phone or TV, it touches both edges. The video sits in a 16px
  sliver, out of sight. Browsers that allow sound get sound; the rest get a
  muted start with a pulsing "tap for sound" speaker (their rule, not ours -
  the bar degrades into a proper radio either way). Opening any of the 12
  watch screens pauses it and hides the bar; closing the screen brings it
  back exactly where it left off. Pages (About/Gallery/Songs/Support) keep
  the music going, as ordered: the bar even floats above them.
- **Time readout fixed everywhere:** MAVO Radio and Bass Radio showed
  "0:152:52" (played and total fused together). Both radios - plus the new
  home bar - now read "0:15 / 2:52" centered under the line.
- **Pick & Play goes loud:** both view-counting screens now carry a pulsing
  green glow border and a "⭐ your press = one real view" chip, so a first-time
  visitor's eye lands where a play actually registers in YouTube Studio.
- **Owner CDLC vault (admin mode, foundation):** Song Search rows now accept
  owner-only download chips - one ⬇ chip per CDLC version, size included,
  each a straight one-click download from files.mavomusic.co.uk with the
  right filename. Unlock = five quick taps on the footer version chip + the
  owner key (lives in localStorage per device, never in the site files);
  "· OWNER" glows in the radio bar once armed. Storage side ships as a kit
  in `_To Attach/ADMIN/`: worker.js (Cloudflare Worker, key-checked R2
  streamer), CDLC-SETUP.md (one-time R2 bucket + worker + rclone guide, with
  the honest price talk: free to 10 GB, ~£2.40/mo at 200 GB, zero download
  fees) and CDLC-Index-Builder.ps1 (scans the CDLC folder, understands
  Artist - Title names + A/B/C/live version tails, writes index.json).
  Nothing changes for visitors until the vault exists - chips simply never
  render without the owner key. Battery clean; jsdom E2E: 14/14 (radio
  shuffle/long-only pool, muted-autoplay fallback, theatre silence, hot
  cards, admin chips with byte-exact href, visitor view untouched).
- **Dash sweep:** number-range en dashes (1-9 keys, 400-900 ms) converted to
  plain hyphens - the site is now 100% hyphen only, zero em dash, zero en
  dash. Chip SITE V2.8.

## V2.7 - 2026-08-23
- **Song Search grew teeth (mod-powered):** it now searches the REAL live request
  library - 48,368 unique active songs built from the owner's StreamerSonglist
  CSV exports into a new `songlist.js` (1.5MB single string, remote-first from
  GitHub like playlists.js, local fallback). Every result row shows
  "Artist - Title", rides with ▶ LONG / ▶ SHORT play chips when a spotlight
  video covers it, and carries a **📋 !sr copy chip** (request command ready to
  paste in stream chat). Songs without a video yet say so ("not uploaded yet -
  request it live 🎤"). Video-to-song meta lives in playlists.js as an optional
  `song: "Artist - Title"` field - auto-linked for 26 of 199 entries (both
  Rammstein twins came home); future batches arrive with band - song included.
  Empty searches explain themselves, 6-taps-or-less copy. jsdom E2E: 9/9 pass
  (owner's own Avril Lavigne example verified byte-exact).
- **Support page, the full arsenal:** free cards stay first (press-play card now
  honest: hand-started plays count, autoplay is atmosphere), then the studio
  fund section - Kick/Twitch/Blaze subs-gifts-cheers, StreamElements + Monzo tip
  links, Merch/Wishlist "soon" teasers, and the complete crypto wall: 11 coin
  rows with networks, click-to-copy addresses, a wired network-safety warning
  and the XRP memo as its own copy chip. One generic data-copy handler powers
  every chip on the site.
- **Copy surgery:** top schedline lost its wordy countdown poetry and the second
  "under the video ↓" - one short line, one plan link. Waitline is a single
  compact sentence (next start in your time + press play). About Me no longer
  says Scunthorpe (stalkers don't get addresses) and gained the real bio: 33,
  born and raised in Hungary, UK since 2014, bass 2017/serious 2019. Gallery
  dropped the Club crest (owner's photos arrive later).
- **Phone nav fix:** the menu pills wrap into two centered rows (3 + 2) instead
  of falling off the screen edge - exactly the split the owner described.
- **The 2.1-ghost:** owner's PC kept getting the ancient cached index from
  Cloudflare's edge while the phone showed V2.6. Fix path = Purge Everything in
  Caching; permanent guard = new `_headers` file in the zip (HTML/playlists/
  songlist = revalidate always; art = 1-day cache). Chip SITE V2.7.

## V2.6 - 2026-08-23
- **The station becomes a website:** top navigation menu - 🏠 HOME · 🎸 ABOUT ME ·
  🖼 GALLERY · 🔍 SONG SEARCH · 💚 SUPPORT - neon pills under the logos, sideways
  sliding on narrow phones, current page glows green. Mod brief: "no excuse not
  to explore".
- **Four content pages, zero new files:** pages open INSIDE index.html as a fixed
  overlay (z above the theatre, same stairway backdrop) from plain-HTML
  <template> blocks the owner can edit like a letter. URLs read #/about style,
  Back button and Esc close, deep links (mavomusic.co.uk/#/songs) open directly.
  Theatre slugs (#marathon) untouched - the two routers ignore each other
  politely. Proven with jsdom: 13/13 (open, hash, close, home, deep link).
- **About Me:** first-person intro, the Rocksmith sight-reading story, 48,000-song
  list fact, cards linking to schedule/streams/videos.
- **Gallery & Media:** station art frames (all 6 assets with captions) + two
  10-tile video walls (long-form / shorts) pulled live from playlists.js,
  reshuffled on every open; thumbnails stream straight from YouTube and quietly
  hide when offline; every tile opens YouTube in a new tab (plays count).
- **Song Search:** one box searches all spotlighted songs (199) as you type,
  LONG/SHORT chips, 40 results max, everything opens on YouTube with the
  "every play counts" note; empty query explains the 48k list lives on stream.
- **How To Support:** six free-things-first cards (watch, follow Kick/Twitch,
  subscribe @MAVOmusicYT, request & chat, share with a copy-the-link button,
  feedback box jump) - honest about no paywalls.
- Page-internal jumps (.jumpplan/.jumpfb) close the page, then drive the
  station's own SCHEDULE/FEEDBACK tabs and glide there. Chip SITE V2.6.
