...
- Song Search dives into the real stream library (V2.7): it reads `songlist.js`,
  a text file generated from your StreamerSonglist CSV exports. New export one
  day → drop both CSVs into `uploads/` and ask for a rebuild, then upload the
  new songlist.js to the GitHub repo. Videos with a `song: "Artist - Title"`
  line in playlists.js show up as play chips on the matching library row.
- The home screen is now a radio too (V2.8, upgraded V2.9): a slim bar at the
  bottom plays random FULL videos (long-form only) while anyone browses -
  MySpace style. It asks the browser for sound; if the browser says no it
  plays quietly until the visitor taps the pulsing speaker. The row carries
  back / pause-resume / next buttons and the clock after the song title; the
  thin line at the very bottom is a scrubber - tap it to jump anywhere in
  the song. Picking any of the 12 screens (or the live stream running)
  silences the radio until the visitor's own play button asks for it again.
- Both Pick & Play cards now pulse green with a "⭐ your press = one real view"
  chip (V2.8) so fresh eyes land where a press actually counts in Studio.
- Owner only: the CDLC vault (V2.8). Five quick taps on the version chip unlock
  download chips on Song Search rows, one per version of a song. The vault
  itself (Cloudflare R2) needs a one-time setup - the full guide, the worker
  code and the index builder live together in `_To Attach/ADMIN/`.
